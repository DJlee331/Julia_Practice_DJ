...
# This code simulates the complete path of the distribution given the shock
...
g1=copy(g_ss);
# println("KFE computations will NOT use the parallel toolbox");
for t=1:nval_Zsim
    if t==1
        # g1=g_ss;
    else
        ...
        # The following block computes the transition g(t+1) = (I - A*dt)^(-1)*g[t] for the four closest grid point
        myA        = sparse(A1[:,:,Int(KposD[t-1,1]), Int(ZposD[t-1,1])]);
        myA        = (spdiagm(ones(nval_a*nval_z))-myA'*dt);
        g1_stacked = myA\vec(g0);
        g1_stacked = g1_stacked/sum(g1_stacked*da);
        g1DD       = reshape(g1_stacked,size(g0));

        myA        = A1[:,:,Int(KposD[t-1,1]),Int(ZposU[t-1,1])];
        myA        = (spdiagm(ones(nval_a*nval_z))-myA'*dt);
        g1_stacked = myA\vec(g0);
        g1_stacked = g1_stacked/sum(g1_stacked*da);
        g1DU       = reshape(g1_stacked,size(g0));

        myA        = A1[:,:,Int(KposU[t-1,1]),Int(ZposD[t-1,1])];
        myA        = (spdiagm(ones(nval_a*nval_z))-myA'*dt);
        g1_stacked = myA\vec(g0);
        g1_stacked = g1_stacked/sum(g1_stacked*da);
        g1UD       = reshape(g1_stacked,size(g0));

        myA        = A1[:,:,Int(KposU[t-1,1]),Int(ZposU[t-1,1])];
        myA        = (spdiagm(ones(nval_a*nval_z))-myA'*dt);
        g1_stacked = myA\vec(g0);
        g1_stacked = g1_stacked/sum(g1_stacked*da);
    ...
    Ksim[t]  =   maximum([Ksim[t] Kmin+0.000001]);
    Ksim[t]  =   minimum([Ksim[t] Kmax-0.000001]);
    KposD[t] = floor.((Ksim[t]-Kmin)/dK)+1;
    KposU[t] =  ceil.((Ksim[t]-Kmin)/dK)+1;
    wK=(K_grid[Int(KposU[t])]-Ksim[t])/dK;         # weight of KposD
