...
@time include("b0_init_ss")

@time include("b1_params.jl")

V1 = (w .* z + r .* a) .^ (1 - γ) / (1 - γ) / ρ;  # utility from always consuming today's income
V1_stacked = vec(V1);
...
Zsim[1] = Z̄;
for t=2:nval_Zsim
    Zsim[t,1] = θ*dt*Z̄+(1-θ*dt)*Zsim[t-1,1] + σ*sqrt(dt)*Zshocks[t];
end
...
for t=1:nval_Zsim
    ...
    wZ[t]    = (Z_grid[Int(ZposU[t])]-Zsim[t])/dZ;   # weight of ZposD
end
...
p = plot(Z_grid,KK_grid,PLM_finegrid,st=:surface)
plot!(p,title="Perceived Law of Motion", xlims=(Zmin, Zmax),ylims=(Kmin, Kmax), zlims=(-0.15, 0.15))
xlabel!("Z")
ylabel!("K")
savefig("fig0.png");
...
#
const X0 = ones(nval_Zsim-delay_Zsim,1);      # we will use this as constant in the just-for-reporting linear regression

diff_vector = ones(maxitPLM,1)*NaN;
...
for it_PLM=1:maxitPLM
    ...
    HJB_solve(~!~!)  #include("b3_HJB.jl")
    println("Simulation started. Total number of years : ",nval_Zsim*dt)
    KFE_simul(~!~!)  # include("b5_KFE.jl")                 
      # Simulate the path of the distribution and aggregate variables for the shocks
